...
START_TEST(test_2d_area_triangle)
{
    ...
    /* all at same point, area should be 0 */
    a.x = b.x = c.x = 0;
    a.y = b.y = c.y = 0;
    double res1 = coord_2d_area_triangle(&a,&b,&c);
    ck_assert(res1 == 0);

    /* when colinear, area should be 0 */
    a.x = 0;
    b.x = 5;
    c.x = 10;
    a.y = b.y = c.y = 0; // all colinear
    double res2 = coord_2d_area_triangle(&a, &b, &c);
    ck_assert(res2 == 0);

    /* right triangle with area of 100 */
    a.x = b.x = a.y = c.y = 5;
    c.x = 25;
    b.y = 15;
